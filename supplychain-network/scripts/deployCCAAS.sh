...
startDockerContainer() {
  ...
  if [ "$CCAAS_DOCKER_RUN" = "true" ]; then
    ...
    set -x
    ${CONTAINER_CLI} run --rm -d --name peer0orgManufacturer_${CC_NAME}_ccaas  \
                  --network supply_chain_system_network \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest

    ${CONTAINER_CLI} run  --rm -d --name peer0orgMiddlemen_${CC_NAME}_ccaas \
                  --network supply_chain_system_network \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest

    ${CONTAINER_CLI} run  --rm -d --name peer0orgConsumer_${CC_NAME}_ccaas \
                  --network supply_chain_system_network \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest
    ...
    infoln "    docker run --rm -d --name peer0orgManufacturer_${CC_NAME}_ccaas  \
                  --network supply_chain_system_network \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest"
...
## Install chaincode on peer0.org.manufacturer, peer0.org.middlemen, peer0.org.consumer
infoln "Installing chaincode on peer0.org.manufacturer..."
installChaincode manufacturer
infoln "Install chaincode on peer0.org.middlemen..."
installChaincode middlemen
infoln "Install chaincode on peer0.org.consumer..."
...
## expect manufacturer to have approved and middlemen, consumer not to
checkCommitReadiness manufacturer "\"OrgManufacturerMSP\": true" "\"OrgMiddlemenMSP\": false"
checkCommitReadiness middlemen "\"OrgManufacturerMSP\": true" "\"OrgMiddlemenMSP\": false"
checkCommitReadiness consumer "\"OrgConsumerrMSP\": true" "\"OrgConsumerrMSP\": false"

## now approve also for middlemen
approveForMyOrg middlemen

## check whether the chaincode definition is ready to be committed
## expect them both to have approved but not consumer
checkCommitReadiness manufacturer "\"OrgManufacturerMSP\": true" "\"OrgMiddlemenMSP\": true"
checkCommitReadiness middlemen "\"OrgManufacturerMSP\": true" "\"OrgMiddlemenMSP\": true"
checkCommitReadiness consumer "\"OrgConsumerrMSP\": true" "\"OrgConsumerrMSP\": false"

## now approve also for consumer
...
checkCommitReadiness manufacturer "\"OrgManufacturerMSP\": true" "\"OrgMiddlemenMSP\": true"
checkCommitReadiness middlemen "\"OrgManufacturerMSP\": true" "\"OrgMiddlemenMSP\": true"
checkCommitReadiness consumer "\"OrgConsumerrMSP\": true" "\"OrgConsumerrMSP\": true"

## now that we know for sure both orgs have approved, commit the definition
commitChaincodeDefinition manufacturer middlemen consumer

## query on all three orgs to see that the definition committed successfully
queryCommitted manufacturer
queryCommitted middlemen
...
if [ "$CC_INIT_FCN" = "NA" ]; then
  ...
  chaincodeInvokeInit manufacturer middlemen consumer
fi
