...
function clearContainers() {
  ...
  ${CONTAINER_CLI} rm -f $(${CONTAINER_CLI} ps -aq --filter name='supplychain-system-user-login') 2>/dev/null || true
}
...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating org-manufacturer Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-org-manufacturer.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
    ...
    infoln "Creating org-middlemen Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-org-middlemen.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
    ...
    infoln "Creating org-consumer Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-org-consumer.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    while :
    do
      if [ ! -f "organizations/fabric-ca/orgManufacturer/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
  ...
  infoln "Generating CCP files for manufacturer, middlemen and consumer"
...
function networkUp() {
  ...
  infoln "Website database loading..."
  ${CONTAINER_CLI} run --name supplychain-system-user-login -p 3306:3306 -e MYSQL_ROOT_PASSWORD=ticketsyspassword -d mysql:oracle
...
# call the script to create the channel, join the peers of orgManufacturer and orgMiddlemen,
...
function networkDown() {
  ...
  #remove wallet folder under supplychain-system-router if exists
  infoln "Removing wallet folder under supplychain-system-router"
  rm -r ${PWD}/../supplychain-system-router/wallet

  # stop orgConsumer containers also in addition to orgManufacturer and orgMiddlemen, in case we were running sample to add org3
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm docker_orderer.example.com docker_peer0.org.manufacturer.example.com docker_peer0.org.middlemen.example.com
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/orgManufacturer/msp organizations/fabric-ca/orgManufacturer/tls-cert.pem organizations/fabric-ca/orgManufacturer/ca-cert.pem organizations/fabric-ca/orgManufacturer/IssuerPublicKey organizations/fabric-ca/orgManufacturer/IssuerRevocationPublicKey organizations/fabric-ca/orgManufacturer/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/orgMiddlemen/msp organizations/fabric-ca/orgMiddlemen/tls-cert.pem organizations/fabric-ca/orgMiddlemen/ca-cert.pem organizations/fabric-ca/orgMiddlemen/IssuerPublicKey organizations/fabric-ca/orgMiddlemen/IssuerRevocationPublicKey organizations/fabric-ca/orgMiddlemen/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/orgConsumer/msp organizations/fabric-ca/orgConsumer/tls-cert.pem organizations/fabric-ca/orgConsumer/ca-cert.pem organizations/fabric-ca/orgConsumer/IssuerPublicKey organizations/fabric-ca/orgConsumer/IssuerRevocationPublicKey organizations/fabric-ca/orgConsumer/fabric-ca-server.db'
...
# TODO remove these when network is working
